...
- Local read source: `/tmp/rp_deepread/zhang2026.txt`
...
这篇论文不应被读成一个普通 affordance segmentation 方法。它真正做的是把 VFM affordance reasoning 拆成可检查的机制问题：模型内部是否已经有足够的 `geometric perception`，是否有可用的 `interaction perception`，以及这两类信号能否组合成 affordance map。

论文的核心命题是：affordance reasoning in VFMs 可以被解释为两个 composable primitives 的组合。一个 primitive 是 object part / spatial layout / geometry cue，另一个 primitive 是 action-conditioned interaction cue。DINO PCA prototype 和 Flux attention 都是观察与桥接这些 primitive 的方法工具，不是论文的最终理论本身。
